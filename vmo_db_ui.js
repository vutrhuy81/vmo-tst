...
Vậy giới hạn cần tìm là $\\sqrt{2}$.`;
  ...
  // Định dạng nội dung Markdown an toàn, chuẩn hóa công thức toán học trần và định dạng các bước rõ ràng
  function formatMathMarkdown(val) {
    ...
    if (typeof val === 'object') {
      return Object.entries(val).map(([k, v]) => `<strong>${k}:</strong> ${formatMathMarkdown(v)}`).join('<div style="margin-top: 6px;"></div>');
    }

    let raw = String(val).trim();
    if (!raw) return '';

    // 1. Chuẩn hóa khoảng trắng & ký tự zero-width
    raw = raw.replace(/[\u200B-\u200D\uFEFF]/g, '').replace(/\u00A0/g, ' ');

    // 2. Tách và bảo vệ các khối Math đã có ($$, $, \[, \()
    const mathTokens = [];
    ...
    // 3. Tách các mục đánh số bị dính liền thành các dòng riêng biệt
    // Ví dụ: "chính xác. 2. Phần (b):" -> "chính xác.\n\n2. Phần (b):"
    text = text.replace(/([.!?])\s+(\d+[\.\)]\s*(?:Phần|Bước|Ý|Trường hợp|[A-ZÀ-Ỹ]))/g, '$1\n\n$2');
    text = text.replace(/(?<!\n)(\b\d+[\.\)]\s*(?:Phần|Bước|Ý|Trường hợp))/g, '\n$1');

    // 4. Chuẩn hóa công thức toán học chưa được bọc dấu $
    // A. Bất đẳng thức & Phương trình chứa số mũ, chỉ số hoặc biểu thức phức tạp trước:
    // Ví dụ: "k >= 3^{k-1}", "m >= n", "k > 1", "k = 1", "x = m", "m = kn", "x = 1", "x = n"
    text = text.replace(/(?<![\w\$])([a-zA-Z0-9\(\)\_\^\{\}\+\-\*\/]+)\s*(>=|<=|=>|==|!=|>|<|=|≥|≤|≠)\s*([a-zA-Z0-9\(\)\_\^\{\}\+\-\*\/]+)(?![\w\$])/g, (m, left, op, right) => {
      if (m.includes('___MATH_BLOCK_')) return m;
      let texOp = op;
      if (op === '>=') texOp = '\\ge ';
      else if (op === '<=') texOp = '\\le ';
    ...
    // B. Các cụm có số mũ ^{...} hoặc chỉ số _{...} hoặc hàm v_p(...) còn lại:
    ...
    // C. Các biểu thức cộng trừ biến số cơ bản: m+1, n+1, x-1
    text = text.replace(/(?<![\w\$\^_{}])([a-zA-Z][\+\-][a-zA-Z0-9]+)(?![\w\$\^_{}])/g, (m) => {
      if (m.includes('___MATH_BLOCK_')) return m;
      return `$${m.trim()}$`;
    });

    // D. Các biến đơn lẻ sau danh từ toán học: "ước nguyên tố p", "với biến x", "ẩn số n"
    ...
    // 5. Chuẩn hóa Markdown: **in đậm**, *in nghiêng*
    text = text
      .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
      .replace(/\*(.*?)\*/g, '<em>$1</em>');

    // 6. Định dạng danh sách đánh số đẹp mắt:
    ...
    // 7. Khôi phục các token Math và chuẩn hóa các ký tự bên trong
    text = text.replace(/___MATH_BLOCK_(\d+)___/g, (match, idx) => {
      let originalMath = mathTokens[parseInt(idx, 10)] || '';
      return originalMath
        .replace(/≥/g, '\\ge ')
        .replace(/≤/g, '\\le ')
        .replace(/≠/g, '\\ne ')
        ...
        <div style="display: flex; align-items: flex-start; gap: 12px; margin-bottom: 12px;">
          ...
          <div>
            ...
            <p style="margin: 0; color: #7f1d1d; font-size: 0.88rem; line-height: 1.5;">
              ${errorMsg || 'Kết nối tới dịch vụ AI gặp trục trặc hoặc model quá tải.'}
            </p>
          </div>
        </div>
